...
{
    public class GrpcServiceAutoMapperProfileStep : CSharpModificationCreatorStep
    {
        protected override IList<ModificationBuilder<CSharpSyntaxNode>> CreateModifications(
            WorkflowExecutionContext context, CompilationUnitSyntax rootUnit)
        {
            var model = context.GetVariable<object>("Model");
            string templateDir = context.GetVariable<string>(VariableNames.TemplateDirectory);

            string usingText = TextGenerator.GenerateByTemplateName(templateDir, "GrpcServicesAutoMapperProfile_Using", model);

            string contents =
                TextGenerator.GenerateByTemplateName(templateDir, "GrpServicesAutoMapperProfile_CreateMap", model);
        ...
        public GrpcServiceAutoMapperProfileStep([NotNull] TextGenerator textGenerator) : base(textGenerator)
        {
        }
    }
}